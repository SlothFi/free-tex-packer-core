import { getPackerByType } from "../packers/index";
import { getExporterByType } from "../exporters/index";
...
export function validateOptions(options) {
  ...
  options.textureName =
    options.textureName === undefined ? "pack-result" : options.textureName;
  options.suffix = options.suffix === undefined ? "-" : options.suffix;
  options.suffixInitialValue =
    options.suffixInitialValue === undefined ? 0 : options.suffixInitialValue;
  ...
  options.fixedSize =
    options.fixedSize === undefined ? false : options.fixedSize;
  ...
  options.allowRotation =
    options.allowRotation === undefined ? true : options.allowRotation;
  options.detectIdentical =
    options.detectIdentical === undefined ? true : options.detectIdentical;
  options.allowTrim =
    options.allowTrim === undefined ? true : options.allowTrim;
  options.trimMode = options.trimMode === undefined ? "trim" : options.trimMode;
  options.alphaThreshold =
    options.alphaThreshold === undefined ? 0 : options.alphaThreshold;
  options.removeFileExtension =
    options.removeFileExtension === undefined
      ? false
      : options.removeFileExtension;
  options.prependFolderName =
    options.prependFolderName === undefined ? true : options.prependFolderName;
  options.textureFormat =
    options.textureFormat === undefined ? "png" : options.textureFormat;
  options.base64Export =
    options.base64Export === undefined ? false : options.base64Export;
  options.scale = options.scale === undefined ? 1 : options.scale;
  options.scaleMethod =
    options.scaleMethod === undefined ? "BILINEAR" : options.scaleMethod;
  ...
  if (!packerMethod) {
    throw new Error(
      getErrorDescription("Unknown packer method " + options.packerMethod)
    );
  }
  ...
  if (!exporter) {
    throw new Error(
      getErrorDescription("Unknown exporter " + options.exporter)
    );
  }
